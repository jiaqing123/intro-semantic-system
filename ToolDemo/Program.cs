...
Console.WriteLine(await kernel.InvokePromptAsync("帮我看看 bob#com 是不是合法邮箱", new(settings)));
Console.WriteLine(await kernel.InvokePromptAsync("3 加 5 等于几？", new(settings)));
Console.WriteLine(await kernel.InvokePromptAsync("今天天气怎么样？", new(settings))); // 不该调工具，直接回答
